...
/**
 * 我他么也想知道这是什么鬼
 *  用于支持多个Mybatis配置文件<br/>
 * 可以把不同模块的MyBatis配置文件分开放置<br/>
 * 是MyBatis配置文件，不是Mapper文件<br/>
 * 主要是配置typeAlias和typeHandler
 * @author Joe
 *
 */
public class MultiConfigSqlSessionFactoryBean extends SqlSessionFactoryBean {


	private static final Log logger = LogFactory.getLog(MultiConfigSqlSessionFactoryBean.class);
	...
	public void setConfigLocations(Resource[] configLocations) throws IOException {
		List<Resource> uniqueLocations = new ArrayList<Resource>();
		StringBuffer sb = new StringBuffer();
		for(Resource r : configLocations){
			if(uniqueLocations.contains(r)){
				logger.error("已经有资源:" + r + "被加载");
				continue;
			}
			logger.error("加载资源:" + r.getURL().toString());
			sb.append(r.getURL().toString()).append(",");
			uniqueLocations.add(r);

		}
		logger.error("加载" + uniqueLocations + "个配置文件:" + sb.toString());
		Resource[]  configLocations2 = new Resource[uniqueLocations.size()];
		configLocations2 = uniqueLocations.toArray(configLocations2);
		this.configLocations  = configLocations2;
	}
	...
	public Document SQLConfigMap() {
		...
		DocumentType docType = documentFactory.createDocType("configuration",
				"-//mybatis.org//DTD SQL Map Config 3.0//EN",
	...
	public void readXML(Resource configXML, final Element elementTypeAlias,
			...
			final Element elementTypeHandler) {
		...
		/* typeHandler合并 */
		saxReader.addHandler("/configuration/typeHandlers/typeHandler",
				new ElementHandler() {
			public void onEnd(ElementPath path) {
				Element row = path.getCurrent();
				Element els = elementTypeHandler.addElement("typeHandler");
				els.addAttribute("javaType", row.attributeValue("javaType"))
				.addAttribute("jdbcType",row.attributeValue("jdbcType"))
				.addAttribute("handler", row.attributeValue("handler"));
				row.detach();
			}
	...
	protected SqlSessionFactory buildSqlSessionFactory() throws IOException {
		...
		Element elementMapper = null;//root.element("mappers");
		Element elementTypeAlias = root.element("typeAliases");
		Element elementTypeHandler = root.element("typeHandlers");
		for (Resource configLocation : configLocations) {
			logger.debug("读取MyBatis配置:" + configLocation.getFilename());
			readXML(configLocation, elementTypeAlias, elementMapper,elementTypeHandler);
		}
