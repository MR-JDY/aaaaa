package cn.ideal.es.common.constants;
...
public enum DataName{
	...
	goodsSmallImage,		
	goodsWeight,					//商品重量
	handlerBackupCardCharge,		//节点是否负责处理备份节点卡密	
	handlerBatchStaticize,		//节点是否负责执行批量静态化任务	
	handlerBossUpdate,			
	handlerForceClosingItem,	//节点是否负责强行关闭交易		
	handlerGiftCardUpdate,		//节点是否负责处理礼品卡	
	handlerGlobalUnique,		//节点是否处理全局唯一数据	
	handlerItem,			
	handlerItemStat,			//节点是否负责处理BOSS数据更新
	handlerJmsDataSyncToLocal,	//节点是否负责把JMS数据同步消息更新到本地		
	handlerJmsHeartBeat,		//节点是否负责回应心跳检测	
	handlerMoney,			
	handlerOperateLog,			
	handlerPay,			
	handlerPayNotify,			//节点是否处理操作日志
	handlerProductUpdate,		//节点是否负责处理产品数据更新	
	handlerQUserInfo,			
	handlerSiteReplicationSlaver,	//节点是否负责站点复制从数据		
	handlerTimeoutAccountCharge,	//节点是否负责超时账户的处理		
	handlerTimeoutCardCharge,	//节点是否负责超时卡密的处理		//节点是否负责响应支付回调
	handlerUserDataUpdate,		//节点是否负责处理用户数据更新	
	handlerUserDynamicDataUpdate,	//节点是否负责更新用户动态数据		
	handlerUserMessageUpdate,	//节点是否负责用户消息数据		//节点是否处理交易统计
	handlerUserUpdate,			
	handlerValidateCacheUpdate,	//节点是否负责验证缓存数据		
	handlerWithdraw,			//节点是否负责处理QQ用户数据
	...
	registerSmsIdelInterval,				//注册验证短信的请求时间间隔
	saleNotifyUrl,			
	soldCount,						//已售出数量
	scoreName,			
	...
	transactionCachePolicy,					//交易订单如何存放在缓存中的规则，@see TransactionCachePolicy;
	tuanActivity,			
	tuanCode,			
	...
	userRegisterNeedSmsSign,				//用户注册时需要提交手机短信验证码
